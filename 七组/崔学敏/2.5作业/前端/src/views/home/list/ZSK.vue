<template>
  <div class="zsk">
    ...
    <div class="bottom" v-for="(item,index) in tableData" :key="index">
      ...
      <el-row>
        <el-button type="primary" @click="edmt(item.id,item.title,item.content)">编辑</el-button>
...
export default {
  ...
  methods: {
    ...
    //编辑跳路由
    edmt(id,title,content){
      console.log(id,title,content)
      this.$router.push({path:'/home/edmtzsk',query:{id,uid:JSON.parse(window.localStorage.user).uid},title,content})
    }
  },
  //获取数据
  async created() {
