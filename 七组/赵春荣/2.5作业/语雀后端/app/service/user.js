...
class UserService extends Service {
    ...
    async addfile(file_name, file_info, know_id, isshow, uid) {
        const $sql = `insert into file (file_name, file_info, know_id, isshow, uid) values ('${file_name}','${file_info}','${know_id}','${isshow}','${uid}')`;
        const result = await this.app.mysql.query($sql);
        return result;
    }
    async delfile(id) {
    ...
    async updatefile(file_name, file_info, know_id, isshow, uid, id) {
        const $sql = ` update file  set file_name='${file_name}', file_info='${file_info}', know_id='${know_id}',isshow='${isshow}', uid='${uid}' where id='${id}'`;
        const result = await this.app.mysql.query($sql);
        return result;
    }
}
