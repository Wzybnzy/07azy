...
class FileController extends Controller {
    async createdoc() {
      ...
      async doclist(){
           let {ctx}=this
           let {uid}=ctx.query
        let res=await ctx.service.file.file.doclist(uid)
        ctx.body={
            code:1,
            data:res
        }
      ...
      async search(){
        let {ctx}=this
        let {input,uid}=ctx.request.body
        let res=await ctx.service.file.file.search(input,uid)
        ctx.body={
            code:1,
            data:res
        }
      ...
      async kdoclist(){
        let {ctx}=this
        let {kid}=ctx.query
        let res=await ctx.service.file.file.kdoclist(kid)
        ctx.body={
            code:1,
            data:res
        }
